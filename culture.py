...
_CULTURE_MESSAGES = [
    ...
    "Keep learning — <https://sites.google.com/a/khanacademy.org/forge/for-khan-employees/conferences|we'll send you to conferences to do so>.",
...
class Culture(webapp2.RequestHandler):
    def get(self):
        """Get request invoked by cron and `/culture` in slack."""

        # In slack, `/culture` can specify a channel.  If not, use the channel
        # it was sent in.  If there is none, we're probably sending from cron,
        # so just use the default.  Note that slack will always send the "text"
        # parameter, even if it's empty.
        channel = (self.request.get("text") or
                   self.request.get("channel_name") or _DEFAULT_CHANNEL)
        if channel[0] != '#':
            channel = '#' + channel
        logging.debug("Sending to channel %s" % channel)

        alertlib.Alert(_get_culture()).send_to_slack(
            channel, sender="Culture Cow", icon_emoji=":cow:")
        logging.debug("Sent")
